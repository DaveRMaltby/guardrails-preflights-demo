...
#          nothing about Normalize's behavior. Because guardrail 01 already proved the build is
...
dotnet test tests/TextTools.Tests --filter "FullyQualifiedName~NormalizerTests" --nologo
if ($LASTEXITCODE -eq 0) {
    Write-Output "the Normalizer tests PASS against the NotImplementedException stub - they are tautological (no real behavior is asserted)"
    exit 1
}
exit 0
